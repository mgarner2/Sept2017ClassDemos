...
public partial class SamplePages_ManagePlaylist : System.Web.UI.Page
{
    ...
    protected void DeleteTrack_Click(object sender, EventArgs e)
    {
        ...
        if (PlayList.Rows.Count == 0)
        {
            MessageUserControl.ShowInfo("Information", "You need to retrieve a play list before deleting.");
        }
        else
        {
            if (string.IsNullOrEmpty(PlaylistName.Text))
            ...
            else
            {
                ...
                for (int i = 0; i < PlayList.Rows.Count; i++)
                {
                    ...
                    if (box.Checked)
                    {
                        //Label gvrow = PlayList.Rows[i].FindControl("TrackId") as Label;
                        //int trackid = int.Parse(gvrow.Text);
